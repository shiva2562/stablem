        ...
        slivers: <Widget>[
          SliverAppBar(
            ...
            flexibleSpace: FlexibleSpaceBar(
              title: Text(name),
              background: pic==''?Image.asset('images/4.jpg',fit: BoxFit.cover,):Image.network(pic,fit: BoxFit.cover,),
            ),

            actions: widget.edit?<Widget>[
              IconButton(icon: Icon(Icons.edit),
                  onPressed: (){
                Navigator.push(context,MaterialPageRoute(builder: (context) => Formm(widget.id,pic,name,role,entryno,email,mobile,whatsapp)),);
                  }),
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      whatsapp==''?infotile('add whatsapp number', Icons.perm_phone_msg):infotile(whatsapp, Icons.perm_phone_msg)
                    ],
                  ),
                ),
...
class Formm extends StatefulWidget{
  ...
  Formm(this.fid,this.fpic,this.fname,this.frole,this.fentryno,this.femail,this.fmobile,this.fwhatsapp);
  @override
                ...
                IconButton(
                  ...
                  onPressed: () async{
                    if(img!=null){
                      ...
                      edpic = await (await uptask.onComplete).ref.getDownloadURL();
                    }
                    _somekey.currentState.save();
                    String fsid = widget.fid;
                    Firestore.instance.document('userdata/$fsid').updateData({'email':edemail,'username':edname,'role':edrole,'pic':edpic,'whatsapp':edwhatsapp,'mobile':edmobile,'entryno':edentryno});
              ...
              child: Column(
                ...
                children: <Widget>[
                    Container(
                      ...
                      width: 250,
                      height: 250,
                      child: img==null ? widget.fpic=='' ? Image.asset('images/4.jpg'):Image.network(widget.fpic):Image.file(
                      img
                    )),
                    RaisedButton(
                      onPressed:(){
